...
inline size_t Ab(const feature_objects& source, const feature_adapter& target, const Transform& t,
                 Eigen::MatrixXf& A, Eigen::VectorXf& b, float* _loss) {
    jacobian_g g;
    init_jacobian_g(g, t);
...
inline auto Ab(std::initializer_list<feature_pair> pairs, const Transform& t = Transform(),
               float* _loss = nullptr) {
...
#pragma omp parallel for
    ...
    Eigen::Matrix<float, 6, 1> ATb = A.topRows(index).transpose() * b.topRows(index);
...
template<typename _Call = nullptr_t>
Transform LM(const feature_objects& source, const feature_objects& target,
    ...
    Eigen::MatrixXf A(total_size, 6);
    Eigen::VectorXf b(total_size);
